...
case class CreateMaterializedViewCommand(viewName: String, query: String)
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val storageLocation = try {
      val desc = sparkSession.sql(s"DESCRIBE EXTENDED $backingTable").collect()
      desc.find(r => r.getString(0).trim == "Location")
        .map(_.getString(1).trim).getOrElse("")
    } catch {
      case _: Exception => ""
    }

    val rowCount = try { sparkSession.table(backingTable).count() } catch { case _: Exception => -1L }
    val sizeInBytes = try {
      val stats = sparkSession.sessionState.catalog
        .getTableMetadata(sparkSession.sessionState.sqlParser.parseTableIdentifier(backingTable))
        .stats
      stats.map(_.sizeInBytes.toLong).getOrElse(-1L)
    } catch {
      case _: Exception => -1L
    }
    ...
    MaterializedViewCatalog.get(viewName) match {
      case Some(meta) =>
        ...
        try {
          ...
          // Update storage info after refresh
          val rowCount = try { sparkSession.table(meta.backingTable).count() } catch { case _: Exception => -1L }
          val storageLocation = try {
            val desc = sparkSession.sql(s"DESCRIBE EXTENDED ${meta.backingTable}").collect()
            desc.find(r => r.getString(0).trim == "Location")
              .map(_.getString(1).trim).getOrElse(meta.storageLocation)
          } catch {
            case _: Exception => meta.storageLocation
          }
          MaterializedViewCatalog.updateStorageInfo(viewName, storageLocation, rowCount, -1L)
        } finally {
    ...
    MaterializedViewCatalog.get(viewName) match {
      case Some(meta) =>
        ...
        try {
          ...
          MaterializedViewCatalog.updateRefreshTs(viewName, System.currentTimeMillis())
          val rowCount = try { sparkSession.table(meta.backingTable).count() } catch { case _: Exception => -1L }
          MaterializedViewCatalog.updateStorageInfo(viewName, meta.storageLocation, rowCount, -1L)
        } finally {
          MaterializedViewOptimizationRule.isDisabled.set(false)
        }
      case None =>
 ...
 * including storage format and location.
 */
case class ShowMaterializedViewsCommand()
  ...
  override val output: Seq[Attribute] = Seq(
    ...
    AttributeReference("storage_format", StringType, nullable = false)(),
    AttributeReference("storage_location", StringType, nullable = false)(),
    AttributeReference("row_count", LongType, nullable = false)(),
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    MaterializedViewCatalog.listAll().map { meta =>
      Row(meta.name, meta.query, meta.backingTable, meta.lastRefreshTs,
          meta.storageFormat, meta.storageLocation, meta.rowCount, meta.sizeInBytes)
    }
  }
