 ...
 *
 * Supports four levels of rewriting (inspired by StarRocks MV capabilities):
 *
 ...
 *
 *  4. '''Join rewrite''' – the query contains JOINs that match the MV's
 *     join structure; the rule replaces the join plan with a scan of the
 *     backing table, with optional predicate compensation and column subset.
 *
...
class MaterializedViewOptimizationRule(session: SparkSession)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    try {
      ...
      // ── 2. Join rewrite ────────────────────────────────────────
      if (JoinRewriter.containsJoin(plan)) {
        val joinResult = JoinRewriter.tryRewrite(session, plan)
        if (joinResult.isDefined) return joinResult.get
      }

      // ── 3. Aggregate / roll-up rewrite ─────────────────────────
